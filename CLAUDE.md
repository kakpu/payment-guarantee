...
コード変更後のデプロイ（`supabase functions deploy` / `supabase db push` / `git push` 等）は、
**変更内容まとめをユーザーに提示し、確認を得てから実行する。**

### 確認が必要な操作
- `supabase functions deploy` — Edge Function のデプロイ
- `supabase db push` — DB マイグレーションの適用
- `git push origin main` — GitHub へのプッシュ（Vercel 自動デプロイが走る）
- その他、本番環境に影響するすべての操作

### 確認フロー
1. コードを変更する
2. **変更内容まとめ**（何を・なぜ変えたか）をユーザーに提示
3. ユーザーの「実行してください」「続けて」等の承認を待つ
4. 承認後にデプロイ・プッシュを実行する
